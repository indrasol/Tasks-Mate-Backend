# app/services/bug_attachment_service.py
import os
import datetime
import re
import mimetypes
...
# Config
ATTACHMENTS_BUCKET = BUG_ATTACHMENTS_BUCKET_TM or "bug-attachments"  # fallback for compatibility
...
async def _generate_sequential_attachment_id() -> str:
    ...
    for _ in range(10):
        ...
        def op():
            return sb.from_("bug_attachments").select("attachment_id").eq("attachment_id", candidate).limit(1).execute()
...
async def _enforce_bug_limit(bug_id: str):
    """Raise 400 if non-deleted attachments >= ATTACHMENTS_LIMIT."""
    sb = get_supabase_client()
    def op():
        return (
            sb.from_("bug_attachments")
    ...
    if count_val >= ATTACHMENTS_LIMIT:
        raise HTTPException(
            status_code=400,
            detail=f"Attachment limit reached ({ATTACHMENTS_LIMIT} per bug)."
        )
...
async def _next_attachment_id_retry(sb, tries=5):
    for _ in range(tries):
        ...
        try:
            ...
            exists = sb.from_("bug_attachments").select("attachment_id").eq("attachment_id", aid).limit(1).execute()
            if not exists.data:
                return aid
        except Exception:
            return aid
...
def _safe_storage_path(prefix: str, original: str, existing: set[str]) -> str:
    """Ensure unique filename under given prefix path (e.g. org_id/bug_id)."""
...
async def upload_and_create_bug_attachment(
    *,
    bug_id: str,
    file: UploadFile,
    title: Optional[str],
    ...
    """
    - Enforce per-bug limit
    - Generate sequential id A0001...
    - Upload to storage: {bug_id}/{original_filename}
    - Fetch public URL
    - Create DB row with the public URL
    - Log 'attachment_created'
    """
    if not is_inline:
        await _enforce_bug_limit(bug_id)
    ...
    # Fetch org_id via project (bugs table has tracker_id)
    try:
        tracker_res = (
            sb.from_("bugs")
        ...
        tracker_id = tracker_res.data.get("tracker_id") or "unknown"
        project_id = tracker_res.data.get("project_id") or "unknown"
    except Exception:
        tracker_id = "unknown"
        project_id = "unknown"

    # Follow pattern: {bucket}/{project_id}/{tracker_id}/{bug_id}/{user_id}/{filename}
    user_id_safe = user_id or "unknown"
    prefix_path = f"{project_id}/{tracker_id}/{bug_id}/{user_id_safe}"
    ...
    try:
        # Ensure unique storage path under this bug
    ...
    def op_insert():
        return sb.from_("bug_attachments").insert(row).execute()

    result = await safe_supabase_operation(op_insert, "Failed to create bug attachment")
...
# # ────────────────────────────────────────────────────────────
# # Existing CRUD: small alignment with
# # ────────────────────────────────────────────────────────────

# async def create_bug_attachment(data: dict, user_id: Optional[str] = None) -> Dict[str, Any]:
#     """Leave available for legacy path (when client already uploaded)."""
#     await _enforce_bug_limit(data["bug_id"])
...
#     sb = get_supabase_client()
#     def op():
#         return sb.from_("bug_attachments").insert(data).execute()

#     result = await safe_supabase_operation(op, "Failed to create bug attachment")

    
#     return result


async def get_bug_attachment(attachment_id: str):
    sb = get_supabase_client()
    def op():
        return sb.from_("bug_attachments").select("*").eq("attachment_id", attachment_id).single().execute()
    return await safe_supabase_operation(op, "Failed to fetch bug attachment")


async def update_bug_attachment(attachment_id: str, data: dict, user_id: Optional[str] = None, bug_title: Optional[str] = None) -> Dict[str, Any]:
    # current = await get_bug_attachment(attachment_id)
    ...
    sb = get_supabase_client()
    def op():
        return sb.from_("bug_attachments").update(data).eq("attachment_id", attachment_id).execute()

    result = await safe_supabase_operation(op, "Failed to update bug attachment")
...
async def delete_bug_attachment(
    ...
    if soft_delete:
        ...
        def op():
            return sb.from_("bug_attachments").update(delete_data).eq("attachment_id", attachment_id).execute()
    else:
        def op():
            return sb.from_("bug_attachments").delete().eq("attachment_id", attachment_id).execute()

    result = await safe_supabase_operation(op, "Failed to delete bug attachment")

    
    return result


async def list_bug_attachments(bug_id: str):
    sb = get_supabase_client()
    def op():
        return (
            sb.from_("bug_attachments")

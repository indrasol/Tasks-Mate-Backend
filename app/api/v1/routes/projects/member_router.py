...
from app.models.schemas.project_member import ProjectMemberCreate, ProjectMemberUpdate, ProjectMemberInDB
from app.services.project_member_service import create_project_member, get_project_member, update_project_member, delete_project_member, get_members_for_project
...
async def update_member(user_id: str, project_id: str, member: ProjectMemberUpdate, user=Depends(verify_token), role=Depends(project_rbac)):
    ...
    result = await update_project_member(user_id, project_id, {**member.dict(exclude_unset=True), "updated_by": user["username"]})
    return result.data[0]
...
async def delete_member(user_id: str, project_id: str, user=Depends(verify_token), role=Depends(project_rbac)):
    ...
    await delete_project_member(user_id, project_id, {"deleted_by": user["username"]})
    return {"ok": True}